...
async function prepareAddressForDummyUtxos({ address, network, pubKey, feeRate, addressType, utxos = [], nOffers = 1 }) {
    try {
        const paddingUtxos = getAllUTXOsWorthASpecificValue(utxos, 600);
        const nDummyUtxos = (nOffers + 1) - paddingUtxos.length;
        if (nDummyUtxos > 0) {
            return dummyUtxosPsbt({ address, utxos, network, feeRate, pubKey, addressType, nDummyUtxos });
        }
        return null;
    }
    catch (err) {
...
function dummyUtxosPsbt({ address, utxos, feeRate, pubKey, addressType, network, nDummyUtxos }) {
    const amountNeeded = ((exports.DUMMY_UTXO_SATS * nDummyUtxos) + parseInt((exports.ESTIMATE_TX_SIZE * feeRate).toFixed(0)));
    const retrievedUtxos = getUTXOsToCoverAmount({
        utxos,
        amountNeeded,
    });
    ...
    return buildPsbtWithFee({
        ...
        spendPubKey: pubKey,
        amountRetrieved,
        spendAmount: (exports.DUMMY_UTXO_SATS * nDummyUtxos),
        feeRate,
        network,
        addressType
    });
