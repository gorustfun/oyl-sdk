...
exports.deployCommit = exports.createTransactReveal = exports.executeReveal = exports.actualTransactRevealFee = exports.findAlkaneUtxos = exports.createDeployReveal = exports.createDeployCommit = exports.execute = exports.actualExecuteFee = exports.createExecutePsbt = void 0;
...
const createDeployCommit = async ({ payload, gatheredUtxos, 
// tweakedTaprootKeyPair,
tweakedPublicKey, account, provider, feeRate, fee, }) => {
    try {
        const originalGatheredUtxos = gatheredUtxos;
...
const createDeployReveal = async ({ createReserveNumber, receiverAddress, script, feeRate, 
// tweakedTaprootKeyPair,
tweakedPublicKey, provider, fee = 0, commitTxId, }) => {
    try {
        if (!feeRate) {
            feeRate = (await provider.esplora.getFeeEstimates())['1'];
        }
...
const actualTransactRevealFee = async ({ calldata, 
// tweakedTaprootKeyPair,
...
const createTransactReveal = async ({ calldata, receiverAddress, script, feeRate, 
//tweakedTaprootKeyPair,
tweakedPublicKey, provider, fee = 0, commitTxId, }) => {
    try {
        if (!feeRate) {
...
const deployCommit = async ({ payload, gatheredUtxos, account, provider, feeRate, signer, }) => {
    ...
    // Get just the tweaked public key as hex string
    const tweakedPublicKey = tweakedTaprootKeyPair.publicKey.toString('hex');
