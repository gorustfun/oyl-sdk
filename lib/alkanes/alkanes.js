...
const createSendPsbt = async ({ gatheredUtxos, account, alkaneId, provider, toAddress, amount, feeRate, fee, }) => {
    try {
        ...
        gatheredUtxos = (0, __1.findXAmountOfSats)(originalGatheredUtxos.utxos, Number(finalFee) + Number(__1.inscriptionSats));
        if (gatheredUtxos.utxos.length > 1) {
            ...
            gatheredUtxos = (0, __1.findXAmountOfSats)(originalGatheredUtxos.utxos, Number(finalFee) + Number(amount));
        }
        ...
        for await (const utxo of alkaneUtxos) {
            if ((0, __2.getAddressType)(utxo.address) === 0) {
            ...
            if ((0, __2.getAddressType)(utxo.address) === 2) {
            ...
            if ((0, __2.getAddressType)(utxo.address) === 1 ||
                (0, __2.getAddressType)(utxo.address) === 3) {
                psbt.addInput({
                    hash: utxo.txId,
                    index: parseInt(utxo.txIndex),
                    witnessUtxo: {
        ...
        if (gatheredUtxos.totalAmount < finalFee + __1.inscriptionSats * 2) {
            throw new errors_1.OylTransactionError(Error('Insufficient Balance'));
        }
        for (let i = 0; i < gatheredUtxos.utxos.length; i++) {
            if ((0, __2.getAddressType)(gatheredUtxos.utxos[i].address) === 0) {
            ...
            if ((0, __2.getAddressType)(gatheredUtxos.utxos[i].address) === 2) {
                const redeemScript = bitcoin.script.compile([
                    bitcoin.opcodes.OP_0,
                    bitcoin.crypto.hash160(Buffer.from(account.nestedSegwit.pubkey, 'hex')),
                ]);
            ...
            if ((0, __2.getAddressType)(gatheredUtxos.utxos[i].address) === 1 ||
                (0, __2.getAddressType)(gatheredUtxos.utxos[i].address) === 3) {
                psbt.addInput({
                    hash: gatheredUtxos.utxos[i].txId,
                    index: gatheredUtxos.utxos[i].outputIndex,
                    witnessUtxo: {
        ...
        const changeAmount = gatheredUtxos.totalAmount +
            totalSatoshis -
            (finalFee + __1.inscriptionSats * 2);
        psbt.addOutput({
...
const createExecutePsbt = async ({ gatheredUtxos, account, calldata, provider, feeRate, fee = 0, }) => {
    try {
        ...
        gatheredUtxos = (0, __1.findXAmountOfSats)(originalGatheredUtxos.utxos, Number(finalFee));
        let psbt = new bitcoin.Psbt({ network: provider.network });
...
const createDeployCommit = async ({ payload, gatheredUtxos, tweakedTaprootKeyPair, account, provider, feeRate, fee, }) => {
    try {
        ...
        gatheredUtxos = (0, __1.findXAmountOfSats)(originalGatheredUtxos.utxos, 20000 + Number(__1.inscriptionSats));
        ...
        if (gatheredUtxos.totalAmount < finalFee + __1.inscriptionSats) {
            throw new errors_1.OylTransactionError(Error('Insufficient Balance'));
        }
        const changeAmount = gatheredUtxos.totalAmount - (finalFee + 20000 + __1.inscriptionSats);
        psbt.addOutput({
            address: account[account.spendStrategy.changeAddress].address,
            value: changeAmount,
        });
...
const createDeployReveal = async ({ createReserveNumber, receiverAddress, script, feeRate, tweakedTaprootKeyPair, provider, fee = 0, commitTxId, }) => {
    try {
        ...
        const commitTxOutput = await (0, __1.getOutputValueByVOutIndex)({
            txId: commitTxId,
            vOut: 0,
...
const deployCommit = async ({ payload, gatheredUtxos, account, provider, feeRate, signer, }) => {
    const tweakedTaprootKeyPair = (0, __1.tweakSigner)(signer.taprootKeyPair, {
        network: provider.network,
    });
