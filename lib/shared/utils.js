...
const calculateUtxoFeesAndGather = ({ gatheredUtxos, feeRate, fee, }) => {
    ...
    let updatedUtxos = findXAmountOfSats(originalGatheredUtxos.utxos, Number(finalFee) + Number(exports.inscriptionSats));
    if (updatedUtxos.utxos.length > 1) {
        ...
        updatedUtxos = findXAmountOfSats(originalGatheredUtxos.utxos, Number(finalFee) + Number(exports.inscriptionSats));
    }
    if (updatedUtxos.totalAmount < finalFee + exports.inscriptionSats * 2) {
        throw new errors_1.OylTransactionError(Error('Insufficient Balance'));
    }
    return { updatedUtxos, finalFee };
};
exports.calculateUtxoFeesAndGather = calculateUtxoFeesAndGather;
