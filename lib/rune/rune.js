...
const createEtchCommit = async ({ gatheredUtxos, tweakedPublicKey, runeName, account, provider, feeRate, fee, }) => {
    try {
        const originalGatheredUtxos = gatheredUtxos;
        const minFee = (0, btc_1.minimumFee)({
            taprootInputCount: 2,
        ...
        let script = [
            (0, bip371_1.toXOnly)(Buffer.from(tweakedPublicKey, 'hex')),
...
const createEtchReveal = async ({ symbol, cap, premine, perMintAmount, turbo, divisibility, runeName, receiverAddress, script, feeRate, tweakedPublicKey, provider, fee = 0, commitTxId, account, }) => {
    try {
        if (!feeRate) {
        ...
        const p2pk_redeem = { output: Buffer.from(script, 'hex') };
...
const actualSendFee = async ({ gatheredUtxos, account, runeId, provider, inscriptionAddress = account.taproot.address, toAddress, amount, feeRate, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
...
const actualMintFee = async ({ gatheredUtxos, account, runeId, provider, feeRate, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
...
const actualEtchCommitFee = async ({ tweakedPublicKey, gatheredUtxos, account, runeName, provider, feeRate, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
...
const actualEtchRevealFee = async ({ tweakedPublicKey, symbol, cap, premine, perMintAmount, turbo, divisibility, runeName, commitTxId, receiverAddress, script, account, provider, feeRate, }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
    }
