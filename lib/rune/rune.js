...
const createEtchCommit = async ({ gatheredUtxos, 
//taprootKeyPair,
//tweakedTaprootKeyPair,
tweakedPublicKey, runeName, account, provider, feeRate, fee, }) => {
    try {
        const originalGatheredUtxos = gatheredUtxos;
        const minFee = (0, btc_1.minimumFee)({
            taprootInputCount: 2,
...
const createEtchReveal = async ({ symbol, cap, premine, perMintAmount, turbo, divisibility, runeName, receiverAddress, script, feeRate, 
//tweakedTaprootKeyPair,
tweakedPublicKey, provider, fee = 0, commitTxId, account, }) => {
    try {
        if (!feeRate) {
...
const actualEtchCommitFee = async ({ 
//tweakedTaprootKeyPair,
tweakedPublicKey, 
//taprootKeyPair,
gatheredUtxos, account, runeName, provider, feeRate,
//signer,
 }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
...
const actualEtchRevealFee = async ({ 
//tweakedTaprootKeyPair,
tweakedPublicKey, 
//taprootKeyPair,
symbol, cap, premine, perMintAmount, turbo, divisibility, runeName, commitTxId, receiverAddress, script, account, provider, feeRate,
//signer,
 }) => {
    if (!feeRate) {
        feeRate = (await provider.esplora.getFeeEstimates())['1'];
